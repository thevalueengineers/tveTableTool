...
test_that("calculate_means works with column breakdown", {
  ...
  expect_true(ncol(new_out) > 3) # Should have additional columns for breakdown
  
  # Check that totals match the non-breakdown version
  total_means <- calculate_means(input_data = test_list$loaded_data[, c(1, 4, 7:8), with = F],
                               respid_var = 'respid',
                               var_labels = test_list$var_labels,
                               val_labels = test_list$val_labels)
  expect_equal(new_out$Total, total_means$Total)
})

test_that("calculate_means works with custom weights", {
  # Add a custom weight column
  test_data <- copy(test_list$loaded_data[, c(1, 4, 7:8), with = F])
  test_data[, custom_weight := runif(.N)]
  
  new_out <- calculate_means(input_data = test_data,
                            respid_var = 'respid',
                            var_labels = test_list$var_labels,
                            val_labels = test_list$val_labels,
                            weight_var = 'custom_weight')

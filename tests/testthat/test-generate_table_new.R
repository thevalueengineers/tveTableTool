...
#single code only
temp_single_table <- generate_table_new(
  test,
  row_vars = "agegroup",
  col_var = "s2",
  weight_var = "gender_weight1",
  variable_labels = tveDataLoader::get_varLabels(test),
  value_labels = tveDataLoader::get_valLabels(test)
)

#multi code only
temp_multi_table <- generate_table_new(
  test,
...
#numeric only
temp_numeric_table <- generate_table_new(
  test,
  row_vars = "agegroup",
  col_var = "s3",
  weight_var = "gender_weight1",
...
temp_combined_table <- generate_table_new(
  test,
...
temp_combined_table_unweighted <- generate_table_new(
  test,
  row_vars = c(paste0("s1_", sprintf("%02d", c(01:08, 99))),"s2", "s3"),
  col_var = "s2",
  weight_var = NULL,
